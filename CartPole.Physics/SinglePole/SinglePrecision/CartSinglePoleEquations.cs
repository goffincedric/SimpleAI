...
namespace CartPolePhysics.SinglePole.SinglePrecision
{
    ...
    public class CartSinglePoleEquations
    {
        ...
        /// </summary>
        public CartSinglePoleEquations(float g, float m, float m_c, float l, float mu_p, float mu_c)
        {
            this.g = g;
            this.m = m;
            this.m_c = m_c;
        ...
        public void CalcAccelerations(float[] state, float f, out float xa, out float thetaa)
        {
            ...
            xa =
                (
                    ...
                    - (7f / 3f) * (f + m * l_hat * thetav_sqr * sin_theta - mu_c * xv)
                    - ((mu_p * thetav * cos_theta) / l_hat)
                ) / (m * cos_theta_sqr - (7f / 3f) * M);

            // Calc pole angular acceleration.
            thetaa =
                (3f / (7f * l_hat))
                * (g * sin_theta - xa * cos_theta - (mu_p * thetav) / (m * l_hat));
        }
