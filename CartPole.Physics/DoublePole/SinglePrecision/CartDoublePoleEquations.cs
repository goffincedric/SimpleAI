...
namespace CartPolePhysics.DoublePole.SinglePrecision
{
    ...
    public class CartDoublePoleEquations
    {
        ...
        )
        {
            ...
            xa =
                (
                    g * ((m * sin_theta * cos_theta) + (m2 * sin_theta2 * cos_theta2))
                    - (7f / 3f)
                        * (
                            f
                            + (m * l_hat * thetav_sqr * sin_theta)
                            + (m2 * l2_hat * thetav2_sqr * sin_theta2)
                            - mu_c * xv
                        )
                    - (
                        ((mu_p * thetav * cos_theta) / l_hat)
                        + ((mu_p * thetav2 * cos_theta2) / l2_hat)
                    )
            ...
            thetaa2 =
                (3f / (7f * l2_hat))
                * (g * sin_theta2 - xa * cos_theta2 - ((mu_p * thetav2) / (m2 * l2_hat)));
        }
